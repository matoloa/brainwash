...
3. **Architectural Guidelines (Especially for UI/Stats Layer)**
   ...
   - **State & Singletons**: `uistate` (from `ui_state_classes.py`) is the source of truth. `UIsub` sets `self.uistate` / `self.config` / `self.uiplot`; migrated mixins (`StatTestMixin`, `SelectionMixin`, `DataFrameMixin`) use `self.*` — no module-level injection.
