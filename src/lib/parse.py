...
def parse_ibwFolder(folder, dev=False, gain=1.0, progress_callback=None):  # igor2, para
...
def parse_csvFolder(folder_path):
    ...
    print(f" - - parse_csvFolder: loaded {len(result)} CSV file(s) from '{folder_path}'.")
    return result
...
def source2dfs(
    ...
    if path.is_dir():  # TODO: currently reads only one type of file:
        ...
        print(f" - - {len(abf_files)} abf files, {len(ibw_files)} ibw files, " f"{len(csv_files)} csv files, and {len(atf_files)} atf files.")
        if csv_files:
            try:
