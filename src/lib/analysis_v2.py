...
def build_dfoutput(df, dict_t, filter="voltage", quick=False):
    ...
    t0 = time.time()
    print(
        f"build_dfoutput: entered, df.shape={df.shape}, nsweeps={df['sweep'].nunique()}"
    )
    ...
    # EPSP_amp
    print(f"build_dfoutput: EPSP_amp section")
    if "t_EPSP_amp" in dict_t.keys():
        ...
        if valid(t_EPSP_amp):
            ...
            print(
                f"build_dfoutput: EPSP_amp computed in {round((time.time() - t0) * 1000)}ms"
            )
            # Normalize EPSP_amp
    ...
    # EPSP_slope
    print(f"build_dfoutput: EPSP_slope section")
    if "t_EPSP_slope_start" in dict_t.keys():
        ...
        print(
            f"build_dfoutput: EPSP_slope t_start={t_EPSP_slope_start}, t_end={t_EPSP_slope_end}"
        )
        if (
            valid(t_EPSP_slope_start, t_EPSP_slope_end)
            and t_EPSP_slope_start < t_EPSP_slope_end
        ):
            ...
            print(
                f"build_dfoutput: EPSP_slope computed in {round((time.time() - t0) * 1000)}ms"
            )
    ...
    # volley_amp
    print(f"build_dfoutput: volley_amp section")
    if "t_volley_amp" in dict_t.keys():
        ...
        print(
            f"build_dfoutput: volley_amp done in {round((time.time() - t0) * 1000)}ms"
        )
    # volley_slope
    print(f"build_dfoutput: volley_slope section")
    if "t_volley_slope_start" in dict_t.keys():
        ...
        print(
            f"build_dfoutput: volley_slope t_start={t_volley_slope_start}, t_end={t_volley_slope_end}"
        )
        if (
            valid(t_volley_slope_start, t_volley_slope_end)
            and t_volley_slope_start < t_volley_slope_end
        ):
            df_volley_slope = measureslope_vec(
                df=df,
                filter=filter,
                t_start=t_volley_slope_start,
                t_end=t_volley_slope_end,
            )
            dfoutput["volley_slope"] = -df_volley_slope["value"]  # invert
            print(
                f"build_dfoutput: volley_slope computed in {round((time.time() - t0) * 1000)}ms"
            )
        else:
            dfoutput["volley_slope"] = np.nan

    print(
        f"build_dfoutput: done {round((time.time() - t0) * 1000)}ms, list_col: {list_col}"
    )
...
def find_events(
    dfmean, default_dict_t, i_stims=None, stim_amp=0.005, precision=None, verbose=False
):
    """
    ...
    def i2t(
        ...
        amp_zero = (
            0  # TODO: placeholder for debugging, remove when amp_zero is implemented
        )
        ...
        if stim_char.get("volley_detected"):
            ...
            t_volley_amp = (
                df_event_range.iloc[i_trough]["time"]
                if i_trough is not None
                else t_stim + 0.0007
            )
            ...
            if isinstance(t_volley_slope_start, np.ndarray):
                ...
                print(
                    "--- t_volley_slope_start NOT <numpy.ndarray> (check characterize_graph for consistency)"
                )
        ...
        if stim_char.get("epsp_detected"):
            ...
            t_EPSP_amp = (
                df_event_range.iloc[i_epsp_min]["time"]
                if i_epsp_min is not None
                else t_stim + 0.005
            )
            ...
            if isinstance(t_EPSP_slope_start, np.ndarray):
                ...
                print(
                    "--- t_EPSP_slope_start NOT <numpy.ndarray> (check characterize_graph for consistency)"
                )
        ...
        t_volley_slope_end = round(
            t_volley_slope_start + default_dict_t["t_volley_slope_width"], precision
        )
        t_EPSP_slope_end = round(
            t_EPSP_slope_start + default_dict_t["t_EPSP_slope_width"], precision
        )
    ...
    def unwrap(v):
        if isinstance(v, pd.Series):
            if len(v) == 1:
                ...
                raise ValueError(
                    f"Expected scalar or single-element Series, got Series with {len(v)} elements: {v}"
                )
        return v
    ...
    min_interval_samples = (
        200  # 20 ms at 10 kHz to ensure no overlap in up to 50Hz trains
    )
...
def characterize_graph(df, stim_amp=0.005, verbose=False, plot=False, multiplots=False):
    ...
    stim_prom = stim_amp
    while (
        (len(pos_peaks) == 0 or len(neg_peaks) == 0) and stim_prom > 1e-6
    ):  # iterative stim search until at least 2 prominent peaks are found
    ...
    if neg_peaks.size > 0 and pos_peaks.size > 0:
        ...
        if next_pos is not None:
            ...
            if (times[next_pos] - times[first_neg]) < 0.0004:  # Within 0.4 ms
                ...
                i_stim_neg = (
                    int(first_neg) - 1
                )  # yes I hardcoded these 1 step adjustments. Hope it stays that way
                i_stim_pos = (
                    int(next_pos) + 1
                )  # yes I hardcoded these 1 step adjustments. Hope it stays that way
    ...
    volley_end = min(
        volley_start + int(0.005 / dt), len(voltage)
    ...
    i_veb = volley_start
    while (
        len(v_peaks) < 2 and v_prom > 1e-6
    ):  # iterative volley search until at least 2 prominent peaks are found
        v_peaks, v_peak_props = find_peaks(volley_region, prominence=v_prom)
        v_prom /= 2
    v_troughs, _ = find_peaks(
        -volley_region, prominence=v_prom
    )  # using same v_prom for peak and trough. seems to work but could change
    i_volley_peaks = [
        p + volley_start for p in v_peaks if p + volley_start < len(voltage)
    ]
    i_volley_troughs = [
        t + volley_start for t in v_troughs if t + volley_start < len(voltage)
    ]
    ...
    epsp_region = voltage[
        epsp_start:epsp_end
    ]  # TODO: could be useful to savgol this for robustness
    if len(epsp_region) > 0:
        ...
        if len(e_peaks):
            i_epsp_min = (
                e_peaks[0] + epsp_start
            )  # just choosing the leftmost now. with normal curves that should be it, unless noise is very bad
    ...
    if m_shape and "i_volley_trough" in locals():
        ...
        if trough - left_peak >= 2:
            slopes = []
            win_length = min(
                (trough + 1 - left_peak) // 2, 3
            )  # set savgol window length
            pad = 3
            voltage_slope = savgol_filter(
                voltage[left_peak : trough + pad], window_length=win_length, polyorder=0
            )
    ...
    if epsp_detected and len(i_volley_peaks) >= 2 and i_epsp_min is not None:
        ...
        if epsp_min - right_peak >= 6:
            r2_values = []
            win_length = min(
                (epsp_min + 1 - right_peak) // 2, 6
            )  # set savgol window length
            pad = 7
            voltage_slope = savgol_filter(
                voltage[right_peak : epsp_min + pad],
                window_length=win_length,
                polyorder=1,
            )
            times_slope = times[right_peak : epsp_min + pad]
            for i in range(
                (len(voltage_slope) - pad) // 4
            ):  # first fourth for slope start
                x = times_slope[i : i + 7]
    ...
    result = {
        ...
        "standard_structure": stim_detected
        and volley_detected
        and m_shape
        and epsp_detected,
    ...
    if plot:
        ...
        if volley_start < len(times) and volley_end <= len(times):
            ...
            for i in i_volley_peaks:
                if i < len(times):
                    plt.plot(
                        ...
                        "go",
                        label="Volley Peak"
                        if "Volley Peak" not in plt.gca().get_legend_handles_labels()[1]
                        else "",
                    )
            ...
            if i is not None and i < len(times):
                plt.plot(
                    ...
                    "mo",
                    label="Volley Trough"
                    if "Volley Trough" not in plt.gca().get_legend_handles_labels()[1]
                    else "",
                )

        # EPSP region and minimum
        if (
            epsp_start is not None
            and epsp_end is not None
            and epsp_start < len(times)
            and epsp_end <= len(times)
        ):
            t_epsp_start = times[epsp_start]
            t_epsp_end = times[min(epsp_end - 1, len(times) - 1)]
            plt.axvspan(
                t_epsp_start, t_epsp_end, color="cyan", alpha=0.2, label="EPSP Region"
            )
        ...
        if i_volley_slope_start is not None and i_volley_slope_end is not None:
            plt.plot(
                times[i_volley_slope_start : i_volley_slope_end + 1],
                voltage[i_volley_slope_start : i_volley_slope_end + 1],
                "r-",
                linewidth=2,
                label="Volley Slope",
            )
        ...
        if i_epsp_slope_start is not None and i_epsp_slope_end is not None:
            plt.plot(
                times[i_epsp_slope_start : i_epsp_slope_end + 1],
                voltage[i_epsp_slope_start : i_epsp_slope_end + 1],
                "r-",
                linewidth=2,
                label="EPSP Slope",
            )
        ...
        padding = (
            0.1 * (feature_max - feature_min) if feature_max != feature_min else 0.001
        )
...
if __name__ == "__main__":
    ...
    for source in list_sources:
        ...
        if dict_events.shape[0] == 1:
            dict_t.update(
                dict_events.iloc[0].to_dict()
            )  # find_events returns a DataFrame
        else:
    ...
    for event in list_event_summary:
        ...
        print(
            f"t_stim: {event['t_stim']}, vS:{event['t_volley_slope_method']}, ES: {event['t_EPSP_slope_method']}, "
    ...
    print(
        f"   Successful volley slope detections: {successful_volley_slope}/{len(list_event_summary)}"
    )
    print(
        f"   Successful EPSP slope detections: {successful_EPSP_slope}/{len(list_event_summary)}"
    )
...
if False:  # __name__ == "__main__":
    ...
    dfresults = dfresults[
        [col for col in dfresults if col.startswith("t_")] + ["sweepname"]
    ]
    dfdiff = dfresults - meta[["t_EPSP_slope_start", "t_volley_slope_start"]]
    dfdiff["sweepname"] = dfresults.sweepname
    worst_list = dfdiff.loc[
        dfdiff.t_EPSP_slope_start.abs().nlargest(5).index
