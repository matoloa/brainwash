...
def build_dfoutput(df, dict_t, filter="voltage", quick=False):
    ...
    """
    t0 = time.time()
    print(f"build_dfoutput: entered, df.shape={df.shape}, nsweeps={df['sweep'].nunique()}")
    ...
    if "t_EPSP_amp" in dict_t.keys():
        ...
        if valid(t_EPSP_amp):
            ...
            print(f"build_dfoutput: EPSP_amp t={t_EPSP_amp}, hw={EPSP_hw}, path={'single' if EPSP_hw == 0 or quick else 'groupby'}")
            if EPSP_hw == 0 or quick:  # single point
                ...
                dfoutput["EPSP_amp"] = (df_EPSP_amp[filter] - amp_zero) * -1000  # invert and convert to mV
            else:  # mean (SLOW)
            ...
            print(f"build_dfoutput: EPSP_amp computed in {round((time.time() - t0) * 1000)}ms")
            # Normalize EPSP_amp
    ...
    if "t_EPSP_slope_start" in dict_t.keys():
        ...
        print(f"build_dfoutput: EPSP_slope t_start={t_EPSP_slope_start}, t_end={t_EPSP_slope_end}")
        if valid(t_EPSP_slope_start, t_EPSP_slope_end) and t_EPSP_slope_start < t_EPSP_slope_end:
            ...
            print(f"build_dfoutput: EPSP_slope computed in {round((time.time() - t0) * 1000)}ms")
    ...
    if "t_volley_amp" in dict_t.keys():
        ...
        if valid(t_volley_amp):
            ...
            print(f"build_dfoutput: volley_amp t={t_volley_amp}, hw={volley_hw}, path={'single' if volley_hw == 0 or quick else 'groupby'}")
            if volley_hw == 0 or quick:  # single point
                ...
                dfoutput["volley_amp"] = (df_volley_amp[filter] - amp_zero) * -1000  # invert and convert to mV
            else:  # mean (SLOW)
        ...
        print(f"build_dfoutput: volley_amp done in {round((time.time() - t0) * 1000)}ms")
    # volley_slope
    ...
    if "t_volley_slope_start" in dict_t.keys():
        ...
        print(f"build_dfoutput: volley_slope t_start={t_volley_slope_start}, t_end={t_volley_slope_end}")
        if valid(t_volley_slope_start, t_volley_slope_end) and t_volley_slope_start < t_volley_slope_end:
            ...
            print(f"build_dfoutput: volley_slope computed in {round((time.time() - t0) * 1000)}ms")
        else:
            dfoutput["volley_slope"] = np.nan

    print(f"build_dfoutput: done {round((time.time() - t0) * 1000)}ms, list_col: {list_col}")
...
def characterize_graph(df, stim_amp=0.005, verbose=False, plot=False, multiplots=False):
    ...
    epsp_depth = 0
    i_epsp_min = None
    epsp_start = i_veb if volley_detected else (i_stim_pos + 2 if i_stim_pos is not None else volley_end)
        ...
        if volley_start < len(times) and volley_end <= len(times):
            ...
            for i in i_volley_peaks:
                if i < len(times):
                    plt.plot(
                        ...
                        voltage[i],
                        "go",
                        label="Volley Peak" if "Volley Peak" not in plt.gca().get_legend_handles_labels()[1] else "",
                    )
            ...
            if i is not None and i < len(times):
                plt.plot(
                    ...
                    voltage[i],
                    "mo",
                    label="Volley Trough" if "Volley Trough" not in plt.gca().get_legend_handles_labels()[1] else "",
                )
