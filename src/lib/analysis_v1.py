# analysis_v1.py
# ---------------------------------------------------------------------------
# DEPRECATED — superseded by analysis_v3.
#
# Status:
#   - build_dfoutput        → superseded by analysis_v3.build_dfoutput
#   - build_dfstimoutput    → superseded by analysis_v3.build_dfbinstimoutput
#   - find_i_EPSP_peak_max  → logic embedded in analysis_v3.find_timepoints
#   - find_i_VEB_prim_peak_max → logic embedded in analysis_v3.find_timepoints
#   - find_i_volley_slope   → logic embedded in analysis_v3.find_timepoints
#   - find_all_i / find_all_t → superseded by analysis_v3.find_events
#   - measureslope          → superseded by analysis_v3._scalar_measureslope
#   - measureslope_vec      → superseded by analysis_v3.measureslope_vec
#
# Do NOT add new imports of this module. Use analysis_v3 for all new code.
# ---------------------------------------------------------------------------
    ...
    for i, t_row in df_t.iterrows():
        ...
        if valid(t_row["t_EPSP_slope_start"]) and valid(t_row["t_EPSP_slope_end"]):
            slope_EPSP = measureslope(
                df=df,
                filter=filter,
                t_start=t_row["t_EPSP_slope_start"],
                t_end=t_row["t_EPSP_slope_end"],
            )
...
def find_i_VEB_prim_peak_max(
    ...
    if verbose:
        print(
            " . . . VEB is between",
            minimum_acceptable_i_for_VEB,
            "and",
            max_acceptable_i_for_VEB,
        )
...
def find_all_i(dfmean, i_stims=None, param_min_time_from_i_stim=0.0005, verbose=False):
    ...
    for i in i_stims:
        ...
        dict_i["i_EPSP_amp"] = find_i_EPSP_peak_max(
            dfmean=dfmean,
            ...
            if dict_i["i_VEB"] is not np.nan:
                ...
                if dict_i["i_EPSP_slope"] is not np.nan:
                    ...
                    if dict_i["i_volley_slope"] is not np.nan:
                        ...
                        ].idxmin()  # TODO: make proper function
...
def find_all_t(
    ...
    default_dict_t,
    precision=None,
    param_min_time_from_i_stim=0.0005,
    verbose=False,
):
    """
    ...
    def i2t(index, dfmean, row, precision, default_dict_t):
        ...
        t_EPSP_slope_start = (
            dfmean.loc[row["i_EPSP_slope"] - ((i_EPSP_slope_width - 1) // 2) - 2].time
