...
def parse_ibwFolder(folder, dev=False):  # igor2, para
    ...
    df.columns = np.round(
        np.arange(num_columns) * timestep, int(-np.log(timestep))
    ).tolist()
...
def parse_ibw(filepath, dev=False):  # igor2, para
    files = [
        Path(filepath)
    ]  # TODO: EVERY attempt to read just one filepath, without what should be superfluous lists, have failed in unresolvable mismatch errors
    for file in files:
        if not file.exists():
...
def parseProjFiles(
    dict_folders, df=None, recording_name=None, source_path=None, single_stim=False
):
    """
    ...
    def parser(dict_folders, recording_name, source_path):
        ...
        if Path(source_path).is_dir():
            ...
            list_files = [
                ...
                if -1 < i.find(".ibw") or -1 < i.find(".abf")
            ]
            filetype = None
            ...
            if filetype == "csv":
                ...
                file_base = os.path.splitext(os.path.basename(source_path))[0].replace(
                    ".", "_"
                )
                ...
                dict_sub = {
                    ...
                    "resets": df[
                        (df["sweep_raw"] == df["sweep_raw"].min()) & (df["time"] == 0)
                    ]["sweep"].tolist()[1:],
                }
        ...
        for channel in dfcopy.channel.unique():
            ...
            for i, stim in enumerate(list_stims):
                ...
                if filetype == "abf":  # split df by % nstims
                    ...
                    df_ch_st["sweep"] = (df_ch_st.sweep_raw / nstims).apply(
                        lambda x: int(np.floor(x))
                    )
                elif (
                    filetype == "ibw"
                ):  # split df; time < 0.5 is stim a, time >= 0.5 is stim b
                    if stim == "a":
                ...
                dict_sub = {
                    ...
                    "resets": df_ch_st[
                        (df_ch_st["sweep_raw"] == df_ch_st["sweep_raw"].min())
                        & (df_ch_st["time"] == 0)
                    ]["sweep"].tolist()[1:],
                }
...
if False:  # __name__ == "__main__":  # hardcoded testbed to work with Brainwash Data Source 2023-05-12 on Linux
    ...
    for item in tqdm(list_sources):
        ...
        dict_data_nsweeps = parseProjFiles(
            dict_folders=dict_folders, df=df_files, single_stim=False
        )
        print(
            f" - dict_data_nsweeps: {dict_data_nsweeps}"
        )  # what the parsed file turned into
    t1 = time.time()
