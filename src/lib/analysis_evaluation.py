...
def evaluate_and_report(
    ...
    for col in all_columns:
        if "-" in col:
            ...
            if suffix in [
                ...
                "analytical_volley_start",
                "analytical_epsp_start",
            ]:
    ...
    for method in methods:
        expected_cols = [
            f"{method}-t_EPSP_slope_start",
            f"{method}-t_volley_slope_start",
        ]
        ...
        if missing_cols:
            ...
            analytical_cols = [
                f"{method}-analytical_epsp_start",
                f"{method}-analytical_volley_start",
            ]
...
if __name__ == "__main__":
    ...
    # check_sweep_v1: uses the legacy analysis_v1.find_all_t pipeline for comparison.
    ...
    for sweepname in df.sweepname.unique():
        result = check_sweep_v1(sweepname)
