    ...
    for _, t_row in dft.iterrows():
        ...
        if valid(t_EPSP_amp):
            ...
            if t_EPSP_w == 0 or quick:
                df_pt = dffilter[dffilter["time"] == t_EPSP_amp_f].copy()
                df_pt.reset_index(drop=True, inplace=True)
                az = amp_zero_per_sweep.reindex(df_pt["sweep"].values).fillna(0.0).values
                dfblock["EPSP_amp"] = (df_pt[filter].values - az) * -1000
            else:
        ...
        if valid(t_volley_amp):
            ...
            if t_volley_w == 0 or quick:
                df_pt = dffilter[dffilter["time"] == t_volley_amp_f].copy()
                df_pt.reset_index(drop=True, inplace=True)
                az = amp_zero_per_sweep.reindex(df_pt["sweep"].values).fillna(0.0).values
                dfblock["volley_amp"] = (df_pt[filter].values - az) * -1000
            else:
