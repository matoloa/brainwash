    ...
    def tableProjSelectionChanged(self): # Keep this on top - it gets changed A LOT
        ...
        if len(uistate.rec_select) == 1: # if just one item is selected...
            ...
            if dft.shape[0] > 1:
                ...
                self.tableStimModel.setData(dft)
...
# WIP: TODO: move these to appropriate header in this file
    ...
    def show_tableStim(self, state):
        if state:
            splitter_width = self.h_tableSplitter.size().width()
            splitter_sizes = [splitter_width // 2, self.tableStim.height()]
            self.h_tableSplitter.setSizes(splitter_sizes)
            self.tableStim.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        else:
            self.tableStim.setFixedWidth(0)
    ...
    def onSplitterMoved(self, pos, index):
        ...
        dict_widget = { #key <intermediary name>: value <proper name>
            # h_masterSplitter
                'h_tableSplitter': 'h_tableSplitter_width', # correct
                    'layoutWidget': 'verticalLayoutProj_width',
                    'verticalLayoutWidget': 'verticalLayoutStim_width',
                'layoutWidget_2': 'v_graphSplitter_width',
                    'verticalLayoutWidget_2': 'verticalLayoutMean_height',
                    'verticalLayoutWidget_3': 'verticalLayoutOutput_height',
        }
        # Assuming splitter is the QSplitter that emitted the signal
        splitter = self.sender()
        widget0 = splitter.widget(0)
        widget1 = splitter.widget(1)
        if splitter.objectName() == "v_graphSplitter":
            uistate.splitters[dict_widget[widget0.objectName()]] = widget0.height()
            uistate.splitters[dict_widget[widget1.objectName()]] = widget1.height()
        else:
            uistate.splitters[dict_widget[widget0.objectName()]] = widget0.width()
            uistate.splitters[dict_widget[widget1.objectName()]] = widget1.width()
            
#        print(f"{splitter.objectName()}({splitter.count()}): {widget0.objectName()}:{widget0.width(), widget0.height()}, {widget1.objectName()}:{widget1.width(), widget1.height()}")
#        print(f"{splitter.objectName()}({splitter.count()}): {dict_widget[widget0.objectName()]}:{widget0.width(), widget0.height()}, {dict_widget[widget1.objectName()]}:{widget1.width(), widget1.height()}")
        print(uistate.splitters)
    ...
    def connectUIstate(self): # Connect UI elements to uistate
        ...
        # comboBoxes
        # mods?...
        for splitter_name in ['h_masterSplitter', 'h_tableSplitter', 'v_graphSplitter']:
