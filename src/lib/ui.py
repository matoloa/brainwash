...
class UIsub(
    ...
    def zoomAuto(self, reset=False):
        ...
        uistate.zoom["output_ax1_ylim"] = self._ylim_from_artists(uistate.ax1, ymin=ymin_clamp) or (
            0,
            1.5,
        )
        uistate.zoom["output_ax2_ylim"] = self._ylim_from_artists(uistate.ax2, ymin=ymin_clamp) or (
            0,
            1.5,
        )
    ...
    def connectUIstate(self, disconnect=False):  # ternary (dis)connect of UI elements
        ...
        for splitter_name in ["h_splitterMaster", "v_splitterGraphs"]:
            ...
            (
                ...
                if disconnect
                else splitter.splitterMoved.connect(lambda pos, index, sn=splitter_name: self.onSplitterMoved(sn, pos, index))
            )
