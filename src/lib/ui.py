...
# Disable font rebuild if cache exists (speeds repeat runs)
os.environ.setdefault('MPLSAVE', '1')  # Save cache
...
if debug_mode:
    ...
    try:
        ...
        logger.debug(f"ui.py: script_path: {script_path}")
    except:
        pass
...
class Config:
    def __init__(self):
        self.dev_mode = os.getenv('BRAINWASH_DEBUG', '0') == '1'  # Respect --debug
        print("\n" * 3 + f"{'Development' if self.dev_mode else 'Deploy'} mode - {time.strftime('%H:%M:%S')}")

        clear = False # Clear all caches...
        # ... rest unchanged
...
class UIsub(Ui_MainWindow):
    def __init__(self, mainwindow):
        ...
        logger.debug("super(UIsub, self).__init__() done")
        logger.debug("Calling setupUi...")
        self.setupUi(mainwindow)
        logger.debug("setupUi done")
        if config.verbose:
            print(" - UIsub init, verbose mode")
        logger.debug("Pre-bootstrap")
        self.bootstrap(mainwindow)
        logger.debug("bootstrap done")
        logger.debug("Calling loadProject...")
        self.loadProject()
...
#####################################################
    ...
    def eventDragReleased(self, event, data_x, data_y): # graph release event
        ...
        action_mapping = {
            ...
            "EPSP amp move": ("t_EPSP_amp_method", 'EPSP amp',
                              {'t_EPSP_amp': trow_temp['t_EPSP_amp'], 't_EPSP_amp_halfwidth': trow_temp['t_EPSP_amp_halfwidth'], 'amp_zero': trow_temp['amp_zero']},
                              uistate.updatePointDragZone),
            "volley slope": ("t_volley_slope_method", 'volley slope',
                             {'t_volley_slope_start': trow_temp['t_volley_slope_start'], 't_volley_slope_end': trow_temp['t_volley_slope_end']},
                              uistate.updateDragZones),
            "volley amp move": ("t_volley_amp_method", 'volley amp',
