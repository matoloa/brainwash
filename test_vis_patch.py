...
new_vis = """        aspect = v.get("aspect")
        ...
        label = ""
        if v.get("line"):
            label = getattr(v["line"], "get_label", lambda: "")()
        if "y marker" in label:
            print(f"DEBUG Y MARKER visible check: label={label}, aspect={aspect}, variant={variant}, expected_variant={expected_variant if 'expected_variant' in locals() else 'raw'}")
