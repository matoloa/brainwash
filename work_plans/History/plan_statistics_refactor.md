# Plan: Refactor `statistics.py` into a Modular Stats Package

> **Status**: Plan only — do not implement until approved.  
> **Scope**: `src/lib/statistics.py` (~1498 LOC, single god-dispatcher + 5 test branches + IO).  
> **Goals**: Safety-first incremental split; stable public API; small files agents can edit in isolation.  
> **AGENTS alignment**: No gold-plating; use search_replace after targeted reads; archive to `History/` on completion; keep `compute_statistical_comparison` thin without new abstractions until Phase 0–1 prove they earn their keep.

---

## Problem Statement

`statistics.py` has grown into an unmaintainable monolith. The primary pain point is `compute_statistical_comparison` (~1080 LOC): a sequential chain of `if test_type == ...` branches with overlapping setup, duplicated inner helpers, and order-sensitive guard logic.

### Symptoms (verified in current code — use symbol anchors, not line numbers)

| Issue                                               | Location / evidence                                                                                                                                  |
| --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| God dispatcher                                      | `compute_statistical_comparison` (L413–1492)                                                                                                         |
| `_get_obs` defined 3×                               | L530, L720 (identical); implicit IO ANOVA duplicates inline aggregation L554–560                                                                     |
| `_aggregate_to_unit_level` nested inside dispatcher | L667–696; defined **after** early-return branches that need it                                                                                       |
| FDR logic copy-pasted ~8×                           | `multipletests` in IO ANOVA, RM ANOVA, Friedman, Cluster, Wilcoxon; `_bh_fdr` only in main t-test loop                                               |
| Aspect column resolution duplicated                 | `("amp", "EPSP_amp...")` blocks in every branch                                                                                                      |
| DEBUG `print` left in production                    | Cluster perm. L866–887, L957, L967, L1023                                                                                                            |
| Order-dependent group/variant guards                | L464–488 (Cluster before paired; ANOVA RM before 2-group requirement)                                                                                |
| Mixed concerns                                      | IO regression, implicit IO ANOVA, hierarchy validation, assumption tests, per-test-set loops                                                         |
| No characterization tests                           | Zero pytest coverage; only comment in `analysis_v3.py:1005`                                                                                          |
| **Unreachable branch**                              | IO implicit ANOVA (L518–636) never runs when `experiment_type=="io"` + empty test sets — L504–516 returns IO regression first (see Pre-Flight Audit) |

### Call sites (must remain stable)

- `src/lib/ui.py:62` — `from . import statistics as stats`
- `ui.py:1801` — `_apply_io_regression` → `stats.compute_statistical_comparison(..., experiment_type="io")` (does **not** pass `uistate`; relies on `__self__` recovery L506)
- `ui.py:2109` — `_apply_non_io_test` → same entry point with `experiment_type` from `uistate`
- `ui.py:1983–1984` — IO UI path **never** calls implicit ANOVA branch; always `_apply_io_regression`
- `analysis_v3.py:1005` — comment reference only (no direct import today)

**Public API to preserve** (re-export from facade):

```python
compute_statistical_comparison(...)  # signature unchanged
ttest_per_sweep(...)
_bh_fdr(...)  # private but importable; used in-file and potentially by tests
```

---

## Pre-Flight Audit (verified 2026-07-05)

Investigation findings that must shape implementation — do not assume the plan's intent matches runtime without these checks.

### 1. IO implicit ANOVA is dead code today

Guard order in `compute_statistical_comparison`:

1. L504–516: `if is_io and use_implicit` → **always** `_compute_io_regression_internal` (any `test_type`, including `"ANOVA"`)
2. L518–636: `if test_type == "ANOVA" and use_implicit` → **unreachable** when `is_io` (because `use_implicit` implies `is_io` at L495–496)

UI routing confirms: `apply_statistical_test_if_active` (L1983–1984) sends all IO traffic to `_apply_io_regression`, not the implicit ANOVA branch.

**Refactor rule**: Extract L518–636 anyway (preserves bytes-on-wire if guard order ever changes), but Phase 0 golden test must assert **current** behavior: `experiment_type="io"` + empty test sets → `config["type"] == "IO regression"`, not `"ANOVA"`. Add a separate direct-call test that would hit implicit ANOVA only if L504 guard is removed (mark `@pytest.mark.dead_path` / document as latent).

### 2. Test layout must match project convention

`pyproject.toml` sets `testpaths = ["src/lib"]`. Existing suite: `src/lib/test_parse.py` (unittest style). **Do not** create top-level `tests/stats/` unless `pyproject.toml` is updated in the same PR.

**Target files**:

- `src/lib/test_statistics_fixtures.py` — shared mocks, no assertions
- `src/lib/test_statistics_characterization.py` — golden / contract tests (pytest)

### 3. Optional `statsmodels` dependency
